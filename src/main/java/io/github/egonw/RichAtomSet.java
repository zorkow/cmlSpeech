...
public class RichAtomSet extends RichChemObject implements Iterable<String> {
	...
	public enum Type {
		ALIPHATIC("Aliphatic chain"), FUSED("Fused ring"), ISOLATED("Isolated ring"), SMALLEST("Subring");
	...
	RichAtomSet(IAtomContainer container) {
		super(container);
	}
	...
	/**
	 * Computes positions of atoms or substructures in the atom set. We use the
	 * following heuristical preferences: -- Always start with an element that
	 * has an external bond. -- If multiple external elements we prefer one with
	 * an atom attached (or later with a functional group, as this can be voiced
	 * as substitution).
	 * 
	 * @param offset
	 *            The position offset.
	 * @param globalPositions
	 *            Map of already assigned global positions.
	 *
	 */
	...
	// private void computeAtomPositionsSubstructure(BiMap<Integer, String>
	// globalPositions) {
	// return;
	// }
	...
	public void printPositions() {
		...
		for (Integer key : this.componentPositions.atomPositions.keySet()) {
			System.out.printf("%d\t%d:\t%s\n", key, key + this.offset, this.componentPositions.get(key));
		}
	}
